...
public class ContextTest {
    ...
    public void setUp() {
        ...
        savedQuickContextScanFlag = env.isQuickContextScan();
    }

    @After
    public void tearDown() {
        // Restore the initial value of the quick context scan flag.
        env.setQuickContextScan(savedQuickContextScanFlag);
    }
    ...
    public void testIsEmpty() throws ParseException {
        ...
        QueryBuilder qb = new QueryBuilder(env).setDefs(term);
        Context c = new Context(qb.build(), qb);
        assertFalse(c.isEmpty());

        // Symbol search should be used
        qb = new QueryBuilder(env).setRefs(term);
        c = new Context(qb.build(), qb);
        assertFalse(c.isEmpty());

        // Full search should be used
        qb = new QueryBuilder(env).setFreetext(term);
        c = new Context(qb.build(), qb);
        assertFalse(c.isEmpty());

        // History search should not be used
        qb = new QueryBuilder(env).setHist(term);
        c = new Context(qb.build(), qb);
        assertTrue(c.isEmpty());

        // Path search should not be used
        qb = new QueryBuilder(env).setPath(term);
        c = new Context(qb.build(), qb);
        assertTrue(c.isEmpty());

        // Combined search should be fine
        qb = new QueryBuilder(env).setHist(term).setFreetext(term);
        c = new Context(qb.build(), qb);
        assertFalse(c.isEmpty());
    }
    ...
    private void testGetContext(boolean limit, boolean hitList)
        ...
        env.setQuickContextScan(limit);

        // Search freetext for the term "def"
        QueryBuilder qb = new QueryBuilder(env).setFreetext("def");
        Context c = new Context(qb.build(), qb);
        assertTrue(c.getContext(env, in, out, "", "", "", null, limit,
            qb.isDefSearch(), hits));
        ...
        qb = new QueryBuilder(env).setDefs("def");
        c = new Context(qb.build(), qb);
        assertTrue(c.getContext(env, in, out, "", "", "", defs, limit,
            qb.isDefSearch(), hits));
        ...
        out = hitList ? null : new StringWriter();
        hits = hitList ? new ArrayList<>() : null;
        qb = new QueryBuilder(env).setFreetext("no_match");
        c = new Context(qb.build(), qb);
        assertFalse(c.getContext(env, in, out, "", "", "", null, limit,
            qb.isDefSearch(), hits));
        if (hitList) {
        ...
        out = hitList ? null : new StringWriter();
        hits = hitList ? new ArrayList<>() : null;
        qb = new QueryBuilder(env).setHist("abc");
        c = new Context(qb.build(), qb);
        assertFalse(c.getContext(env, in, out, "", "", "", null, limit,
            qb.isDefSearch(), hits));
        if (hitList) {
    ...
    public void testLongLineNearBufferBoundary() throws ParseException {
        ...
        QueryBuilder qb = new QueryBuilder(env).setFreetext("test");
        Context c = new Context(qb.build(), qb);
        StringWriter out = new StringWriter();
        boolean match = c.getContext(env, in, out, "", "", "", null, true,
            qb.isDefSearch(), null);
    ...
    public void testAllLinkWithLongLines() throws ParseException {
        ...
        QueryBuilder qb = new QueryBuilder(env).setFreetext("search_for_me");
        Context c = new Context(qb.build(), qb);
    ...
    public void testMultiLineMatch() throws Exception {
        ...
        QueryBuilder qb = new QueryBuilder(env).setFreetext("\"a b c\"");
        Context c = new Context(qb.build(), qb);
        assertTrue(
                "No match found",
                c.getContext(env, in, out, "", "", "", null, true,
                qb.isDefSearch(), null));
    ...
    public void bug16848() throws Exception {
        ...
        StringWriter out = new StringWriter();
        QueryBuilder qb = new QueryBuilder(env).setFreetext("mixed");
        Context c = new Context(qb.build(), qb);
        assertTrue(c.getContext(env, in, out, "", "", "", null, false,
            qb.isDefSearch(), null));
    ...
    private void bug17582(QueryBuilder builder, int[] lines, String[] tags)
        ...
        assertEquals(lines.length != 0,
            context.getContext(env, in, null, "", "", "", defs, false,
            builder.isDefSearch(), hits));
    ...
    private void searchContextTestHelper(String searchInText, String queryString, String expectWordInContext) throws ParseException {
        ...
        QueryBuilder qb = new QueryBuilder(env).setFreetext(queryString);
        Context c = new Context(qb.build(), qb);
