...
public final class BoundedBlockingObjectPool<T> extends AbstractObjectPool<T>
    ...
    public T get(long timeOut, TimeUnit unit) {
        if (!shutdownCalled) {
            try {
                return objects.pollFirst(timeOut, unit);
    ...
    protected void returnToPool(T t) {
        if (validator.isValid(t)) {
            executor.submit(new ObjectReturner<T>(objects, t, puttingLast));
        }
    }
    ...
    private class ObjectReturner<E> implements Callable<Void> {
        ...
        public ObjectReturner(LinkedBlockingDeque<E> queue, E e,
            boolean puttingLast) {
            this.queue = queue;
            this.e = e;
